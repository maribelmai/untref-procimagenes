...
public class ActividadSegmentacionImagen extends ActividadBasica {
    ...
    private int[] promedioDeColores = null;
    private int[][] matrizSigmas = null;
    ...
    public void segmentar() {
        ...
        promedioDeColores = obtenerPromedioRGB(bitmapOriginal, punto1, punto2);

        for (int i = 0; i < vecesValor; i++) {
        ...
        Bitmap bitmap = ((BitmapDrawable) imageView.getDrawable()).getBitmap();
        Bitmap imagenCopia = bitmap.copy(Bitmap.Config.RGB_565, true);

        for (Point unPoint : lIn) {

            Log.i(TAG, "esta en lin: " + unPoint.x + "," + unPoint.y);
            imagenCopia.setPixel(unPoint.x, unPoint.y, Color.GREEN);
        }

        imageView.setImageBitmap(imagenCopia);
    }

    private void expandir(Bitmap imagen, List<Point> lIn, List<Point> lOut, int[] promedio,
    ...
    private static int calcularFd(int[] promedio, Bitmap imagen, Point posicion) {
        ...
        boolean debeSeguir = sonColoresSimilares(imagen, Color.rgb(promedio[0], promedio[1], promedio[2]), posicion.x, posicion.y);
    ...
    private static boolean sonColoresSimilares(Bitmap imagen, int colorPromedio, int i, int j) {

        int colorEnEsePunto = imagen.getPixel(i, j);

        boolean esSimilar = Math.abs(Color.red(colorEnEsePunto) - Color.red(colorPromedio)) <= 20
                && Math.abs(Color.green(colorEnEsePunto) - Color.green(colorPromedio)) <= 20
                && Math.abs(Color.blue(colorEnEsePunto) - Color.blue(colorPromedio)) <= 20;

        return esSimilar;
    }

    private void contraer(Bitmap imagen, List<Point> lIn, List<Point> lOut, int[] promedio,
    ...
    private void sacarLinNoCorrespondientes(List<Point> lIn, Iterator<Point> iteradorPuntosLin) {
    ...
    private void sacarLoutNoCorrespondientes(List<Point> lOut, Iterator<Point> iteradorPuntosLout2,
                                                    Bitmap imagen) {
